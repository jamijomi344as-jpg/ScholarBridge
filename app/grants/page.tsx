...
export default function GrantsPage() {
  ...
  return (
    <div style={{ backgroundColor: '#F8FAFC', minHeight: '100vh', paddingBottom: '60px', fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ backgroundColor: '#16233F', color: '#fff', padding: '36px 20px', textAlign: 'center' }}>
        <h1 style={{ fontSize: '28px', fontWeight: 'bold', marginBottom: '12px' }}>Mavjud Xalqaro Grantlar Katalogi</h1>
        <p style={{ fontSize: '15px', color: '#94A3B8', maxWidth: '650px', margin: '0 auto 24px auto' }}>
          O'zingizning baholaringiz hamda IELTS/SAT natijalaringizga mos keladigan grantlarni AI orqali topmoqchimisiz?
        </p>
        
        {/* TUGMA */}
        <Link href="/find">
          <button style={{ backgroundColor: '#2563EB', color: '#ffffff', padding: '14px 28px', borderRadius: '8px', border: 'none', fontWeight: 'bold', fontSize: '16px', cursor: 'pointer', boxShadow: '0 4px 14px rgba(37, 99, 235, 0.4)' }}>
            🎯 Mos Grant Topish (Profil bo'yicha)
          </button>
        </Link>
      </div>

      <div style={{ maxWidth: '1200px', margin: '30px auto', padding: '0 20px', display: 'grid', gridTemplateColumns: '260px 1fr', gap: '24px' }}>
              ...
              <div key={grant.id} style={{ backgroundColor: '#fff', borderRadius: '12px', border: '1px solid #E2E8F0', padding: '16px', display: 'grid', gridTemplateColumns: '220px 1fr', gap: '16px', alignItems: 'center' }}>
                ...
                <div>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '13px', color: '#64748B', borderTop: '1px solid #F1F5F9', paddingTop: '8px' }}>
                    ...
                    <span>📅 Deadline: <strong style={{ color: '#EF4444' }}>{grant.deadline}</strong></span>
                  </div>
                </div>
              </div>
            ))}
